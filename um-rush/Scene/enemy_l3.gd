...
func _process(delta: float) -> void:
	...
	if pattern == "wave":
		...
		if parent and parent.get("lanes") != null:
			...
			if parent_lanes.size() >= 3:
				var middle_y = parent_lanes[1] - 300.0
	...
	if parent and parent.get("lanes") != null:
		var parent_lanes = parent.get("lanes")
		if parent_lanes.size() >= 3:
			position.y = clamp(position.y, parent_lanes[0] - 300.0, parent_lanes[2] - 300.0)
...
func get_current_lane_index() -> int:
	...
	var physical_y = position.y + 300.0
...
func _is_in_same_lane(body: Node) -> bool:
	...
	return abs(position.y + 300.0 - player_lane_y) < 120.0
